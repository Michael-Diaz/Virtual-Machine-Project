...
static const int RESERVED_WORDS_NUM = 15;
static const int MAX_INT = 32767;
static const int MAX_CHAR = 11;
...
typedef struct
{
  ...
  char name[10];	// Name up to 11 chars ~DO WE NEED TO CHANGE THIS TO 11?
...
void main(int argc, char **argv)
{
  ...
  while ((buffer = fgetc(ifp)) != EOF)
  {
    ...
    {
      fprintf(ofp, "\n\nError #4 Unrecognized Symbol: %c\n", buffer); // ~CHANGE THIS NUMBER TO THE APPROPRIATE ERROR
      exit(0);
    }
    else
  ...
  // Error checks ~CHECK FOR EXTRA ERRORS IN HW3
